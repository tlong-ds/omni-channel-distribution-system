...
def write_notes(
    ...
    text = [
        ...
        r"",
        r"The product assortment was analyzed across two dimensions using a joint ABC-XYZ matrix based on outbound volume (Quantity) and transaction frequency (Order Frequency) over the last 6 months of 2025.",
        r"",
        ...
        f"\\item \\textbf{{XYZ Variability Thresholds}}: Class X (CV $\\le$ {metadata['xyz_cv_x_max']:.2f}), Class Y (CV $\\le$ {metadata['xyz_cv_y_max']:.2f}), Class Z (CV > {metadata['xyz_cv_y_max']:.2f} or Low Sample)",
        f"\\item \\textbf{{Low-Sample Filter}}: SKUs with fewer than {int(metadata['xyz_min_nonzero_weeks'])} nonzero sales weeks are automatically downgraded to Class Z to prevent statistical skew.",
        r"\end{itemize}",
        r"",
        ...
        r"\end{figure}",
        r"",
        r"\subsubsection{Joint Quantity vs. Frequency Matrix Summary}\label{joint-quantity-vs.-frequency-matrix-summary}",
        ...
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.75\linewidth]{../charts/q11_abc_quantity_frequency_matrix.png}",
        r"\caption{Q1.1 ABC quantity vs. frequency matrix}",
        r"\label{fig:q11-abc-qty-freq-matrix}",
        r"\end{figure}",
        r"",
        ...
        r"The \textbf{Fast-Moving} SKU group is defined as the intersection of Class A by Quantity and Class A by Order Frequency (Class A-A). This group is the primary driver of warehouse operational workload and inventory velocity.",
        r"",
        r"\begin{itemize}",
