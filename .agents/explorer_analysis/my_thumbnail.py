...
# We need to import get_soffice_env or replicate it, let's see what is inside get_soffice_env in office.soffice
# Let's check what office.soffice contains first, or write our own environment helper.
# Let's inspect /Users/bunnypro/Projects/LOGage2026/.agents/skills/pptx/scripts/office/soffice.py to see how it works.
# Wait, we can just run the original script if we copy it and edit the import of defusedxml to xml.dom.minidom!
# Let's write the modified code here.

import os

def get_soffice_env():
    # If there is a special office.soffice, we can import it by adding the skill scripts path to sys.path
    skill_path = Path("/Users/bunnypro/Projects/LOGage2026/.agents/skills/pptx/scripts")
    sys.path.insert(0, str(skill_path))
    from office.soffice import get_soffice_env as orig_get_soffice_env
    return orig_get_soffice_env()
...
def get_slide_info(pptx_path: Path) -> list[dict]:
    with zipfile.ZipFile(pptx_path, "r") as zf:
        ...
        rels_dom = xml.dom.minidom.parseString(rels_content)
        ...
        pres_dom = xml.dom.minidom.parseString(pres_content)
