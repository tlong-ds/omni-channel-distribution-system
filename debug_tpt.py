...
tpt_net = df_dist_net[df_dist_net['Customer Name'].str.contains("TPT", case=False, na=False)]
print(tpt_net[['Customer Name', 'Delivery Address']].to_string(index=False))

print("\nTPT in Distributor General:")
tpt_gen = df_dist_gen[df_dist_gen['Customer Name'].str.contains("TPT", case=False, na=False)]
...
tpt_cleaned = dist_cleaned[dist_cleaned['customer_key'].str.contains("TPT", case=False, na=False)]
print(tpt_cleaned[['customer_key', 'delivery_address', 'customer_match_status', 'customer_key_is_ambiguous']].to_string(index=False))
